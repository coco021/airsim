...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:  # 要是有视频就先读视频
            # Read video
            self.mode = "video"
        ...
        if self.transforms:
            ...
            im = letterbox(im0, self.img_size, stride=self.stride, auto=self.auto)[0]  # padded resize 原图转换成我想要的大小 （640.480.3）
            im = im.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB 长宽通道数，现在把通道数放前面，BGR RGB
            im = np.ascontiguousarray(im)  # contiguous
